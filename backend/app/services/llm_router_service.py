...
class LlmRouterService:
    ...
    async def complete(
        ...
        """Route LLM call to org-configured provider, fallback to Ollama."""
        cfg = await self.get_config()
        provider = cfg.provider if cfg else "ollama"
        model = cfg.model if cfg else (settings.OLLAMA_MODEL or "qwen2.5:7b")
    ...
    async def _call_ollama(
        ...
        try:
            ...
            return LlmCallResult(
                provider="ollama", model=model, content="", tokens_used=0
            )
