...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            ...
            name="ck_drift_reports_severity",
        ),
        sa.ForeignKeyConstraint(["organization_id"], ["organizations.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    )

    op.create_index(op.f("ix_drift_reports_organization_id"), "drift_reports", ["organization_id"], unique=False)
    op.create_index(op.f("ix_drift_reports_metric_name"), "drift_reports", ["metric_name"], unique=False)

    # RLS policies
    for table in ["meta_agent_runs", "meta_conflict_registry", "belief_store", "calibration_profiles", "drift_reports"]:
...
def downgrade() -> None:
    op.drop_index(op.f("ix_drift_reports_metric_name"), table_name="drift_reports")
    op.drop_index(op.f("ix_drift_reports_organization_id"), table_name="drift_reports")
