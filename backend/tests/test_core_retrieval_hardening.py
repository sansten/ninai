...
@pytest.mark.asyncio
async def test_core_search_retries_vector_without_tags_when_tagged_vector_hits_are_empty(monkeypatch):
    # First vector call (with tags) yields no hits; second call (without tags) recovers.
    qdrant_search = AsyncMock(
        side_effect=[
            [],
            [{"id": "v1", "score": 0.91, "payload": {"memory_id": "m1"}}],
        ]
    )
    monkeypatch.setattr(memory_service_module.QdrantService, "search", qdrant_search)
    monkeypatch.setattr(memory_service_module.settings, "SEARCH_QUERY_EXPANSION_ENABLED", False, raising=False)
    ...
    mem = SimpleNamespace(
        ...
        content_preview="[Caroline] I am researching adoption agencies this summer.",
        vector_id="v1",
        embedding_model="e",
        scope="personal",
        scope_id=None,
        ...
        source_type=None,
        source_id=None,
        tags=["locomo_001", "locomo-full-20260514-202724-89a02828"],
        extra_metadata={"run_tag": "locomo-full-20260514-202724-89a02828"},
    )
    ...
    assert [m.id for m in results] == ["m1"]
    assert qdrant_search.await_count >= 2
    # Initial call uses strict tags; retry call relaxes vector tag filter.
    assert qdrant_search.await_args_list[0].kwargs.get("tags") == req.tags
    assert qdrant_search.await_args_list[1].kwargs.get("tags") is None

    diagnostics = svc.get_last_search_diagnostics()
    assert diagnostics.get("vector_tag_filter_relaxed") is True
    assert diagnostics.get("fallback_reason") == "vector_tag_filter_relaxed_retry"
